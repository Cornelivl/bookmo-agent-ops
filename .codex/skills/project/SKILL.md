---
name: project
description: Create, resume, replan, and close long-running project work using a single tracker at `docs/projects/PROJECT/tasks.md`. Use when starting a new project, continuing an existing project, refreshing a stale plan, or asking an agent to handle a complicated multi-session task with durable repo memory, explicit behavior contracts, skeptical review, milestone validation, and a clear resume point.
---
...
Use one project tracker file as the durable source of truth for long-running work. The tracker is intentionally a hybrid: it should capture the strong planning discipline normally associated with a PRP and the durable execution state normally associated with a task tracker. The skill decides whether to create, resume, replan, or close the tracker based on repo state, keeps that file current while it works, and archives finished projects by default when completion confidence is high.
...
4. **Lock the behavior contract before implementation**
   - Before treating the tracker as execution-ready, write or refresh the planning sections that define the work clearly enough for a cold-start agent:
     - `Behavior Contract`
   ...
   - Treat these sections as non-negotiable for behavior-changing or multi-session work.
   - Ground the contract in current repo reality: current behavior, desired behavior, invariants, side effects, status transitions, rollout constraints, and the evidence that will prove the change.
   - If the work is tiny and purely mechanical, keep these sections concise instead of skipping them entirely.
5. **Run the skeptical review pass**
   - After the first complete draft of the tracker, run a separate review pass focused on hidden risks, edge cases, missing dependencies, unsafe sequencing, weak verification, and ambiguous behavior.
   - If subagents are authorized and useful, delegate the review; otherwise do it locally as a separate pass.
   - Fold the findings back into the tracker before treating it as execution-ready.
   - Record only the durable conclusions in the tracker, not a long review memo by default.
6. **Sync context**
   - Read `Current Batch`, `Progress Log`, `Open Questions / Blockers`, `Milestones`, and `Done When` first.
   - Read the planning sections first as well: `Behavior Contract`, `Desired Flow (Happy Path)`, `Scope / Non-Goals`, `Impacted Areas`, and `Validation / Test Plan`.
   - Read only the code/docs named in `Context / Constraints` or recent progress before scanning the wider repo.
7. **Plan the next execution batch**
...
9. **Checkpoint after each meaningful batch**
...
- Keep planning and execution in the same file unless repo guidance explicitly requires separate artifacts.
- Treat the tracker as both the behavior source of truth and the durable execution board.
...
- Require explicit behavior definitions for meaningful work: current behavior, desired behavior, non-goals, impacted areas, and validation evidence.
- Require a skeptical review pass before treating a new or heavily revised tracker as execution-ready.
...
```text
Review the tracker at <path> as a skeptical implementation reviewer.

Your job is to find what the plan is missing, not to restate it.

Focus on:
- hidden risks and likely failure modes
- edge cases and state transitions not covered by the behavior contract
- missing dependencies, sequencing issues, and unsafe parallelization assumptions
- shared files or modules that make supposed parallel workstreams collide
- missing validation, test coverage, rollout safeguards, or acceptance evidence
- places where the desired behavior is ambiguous, untestable, or internally inconsistent

Return:
1. Findings ordered by severity
2. The exact tracker section each finding should update
3. A concrete patch suggestion for each finding, written as short replacement/addition text
4. A short list of items that are safe as written so the main agent does not over-edit

Be strict. Prefer surfacing a smaller number of high-signal issues over broad commentary.
```
